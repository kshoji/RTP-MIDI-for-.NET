...
namespace SimpleBase
{
    ...
    public sealed class Base85
    {
        ...
        public unsafe string Encode(byte[] bytes)
        {
            ...
            bool usesZeroShortcut = alphabet.AllZeroShortcut.HasValue;
            bool usesSpaceShortcut = alphabet.AllSpaceShortcut.HasValue;
        ...
        {
            StreamHelper.Encode(input, output, (buffer, _) => Encode(buffer));
        }
        ...
        {
            await StreamHelper.EncodeAsync(input, output, (buffer, _) => Encode(buffer))
        ...
        {
            StreamHelper.Decode(input, output, text => Decode(text).ToArray(), decodeBufferSize);
        }
        ...
        {
            await StreamHelper.DecodeAsync(input, output, text => Decode(text).ToArray(), decodeBufferSize)
                .ConfigureAwait(false);
        }
        ...
        public unsafe byte[] Decode(string text)
        {
            ...
            fixed (byte* decodeBufferPtr = decodeBuffer)
            {
                ...
                {
                    result[i] = decodeBufferPtr[i];
                }
                return result;
            }
        }

        private static int getDecodeBufferLength(int textLen, bool usingShortcuts)
        {
            if (usingShortcuts)
            {
