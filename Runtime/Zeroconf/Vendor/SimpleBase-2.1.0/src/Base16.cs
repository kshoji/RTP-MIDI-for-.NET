...
namespace SimpleBase
{
    ...
    public static class Base16
    {
        ...
        {
            StreamHelper.Decode(input, output, buffer => Decode(buffer).ToArray());
        }
        ...
        {
            await StreamHelper.DecodeAsync(input, output, buffer => Decode(buffer).ToArray())
        ...
        {
            StreamHelper.Encode(input, output, (buffer, _) => internalEncode(buffer, 'A'));
        }
        ...
        public static async Task EncodeUpperAsync(Stream input, TextWriter output)
        {
            await StreamHelper.EncodeAsync(input, output, (buffer, _) =>
                internalEncode(buffer, 'A')).ConfigureAwait(false);
        }
        ...
        private static unsafe string internalEncode(byte[] bytes, char baseChar)
        {
