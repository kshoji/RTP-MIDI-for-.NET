...
| `Runtime/Zeroconf/MakaretuZeroconf.cs` | Makaretu `ServiceDiscovery` ラッパ（Advertise / Browse / Resolve） |
| `Runtime/Zeroconf/RtpMidiZeroconfHelpers.cs` | 表示名抽出・自ホスト除外・IPv4 優先 |
...
### Phase 3 — 発見（Browse / Resolve） — 完了 (2026-09-15)

- [x] `StartBrowse` / `StopBrowse`（`QueryServiceInstances("_apple-midi._udp")` を 5 秒周期で実行）
- [x] `ServiceInstanceDiscovered` / `ServiceInstanceShutdown` / `AnswerReceived` を `IRtpMidiServiceDiscoveryListener` にマップ
- [x] SRV → A（優先）/ AAAA の Resolve パイプライン
